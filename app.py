...
# --- CSS VE STYLING (PREMIUM UI & GELİŞMİŞ ANİMASYONLAR) ---
st.markdown("""
<style>
    /* IPHONE VE MOBİL GİRİŞ DÜZELTMESİ */
    ...
    /* LÜKS MAVİ BALON İÇİ 3 NOKTA YÜKLEME ANİMASYONU */
    .typing-dots {
        display: inline-flex;
        align-items: center;
        margin-left: 8px;
...
# --- SIDEBAR (DÜZENLENMİŞ MODERN MENÜ SİSTEMİ) ---
with st.sidebar:
    ...
    # Düğmeler daha simetrik hale getirildi
    ...
    with st.expander("🧮 Fonksiyonel Hesap Makinesi"):
        ...
        st.session_state.calc_val = st.text_input("Ekran:", value=st.session_state.calc_val, key="calc_display")
        
        c1, c2, c3, c4 = st.columns(4)
        if c1.button("7", key="b7"): st.session_state.calc_val += "7"; st.rerun()
        if c2.button("8", key="b8"): st.session_state.calc_val += "8"; st.rerun()
        if c3.button("9", key="b9"): st.session_state.calc_val += "9"; st.rerun()
        if c4.button("/", key="b_div"): st.session_state.calc_val += "/"; st.rerun()
        
        if c1.button("4", key="b4"): st.session_state.calc_val += "4"; st.rerun()
        if c2.button("5", key="b5"): st.session_state.calc_val += "5"; st.rerun()
        if c3.button("6", key="b6"): st.session_state.calc_val += "6"; st.rerun()
        if c4.button("*", key="b_mul"): st.session_state.calc_val += "*"; st.rerun()
        
        if c1.button("1", key="b1"): st.session_state.calc_val += "1"; st.rerun()
        if c2.button("2", key="b2"): st.session_state.calc_val += "2"; st.rerun()
        if c3.button("3", key="b3"): st.session_state.calc_val += "3"; st.rerun()
        if c4.button("-", key="b_sub"): st.session_state.calc_val += "-"; st.rerun()
        
        if c1.button("0", key="b0"): st.session_state.calc_val += "0"; st.rerun()
        if c2.button(".", key="b_dot"): st.session_state.calc_val += "."; st.rerun()
        if c3.button("+", key="b_add"): st.session_state.calc_val += "+"; st.rerun()
        if c4.button("C", key="b_c"): st.session_state.calc_val = ""; st.rerun()
        
        if st.button("=", use_container_width=True, key="b_eq"):
            try: st.session_state.calc_val = str(eval(st.session_state.calc_val))
            except: st.session_state.calc_val = "Hata"
            st.rerun()

# --- MESAJLARI GÖSTERME (RENDER ENGINE) ---
def render_message(msg):
    if msg["role"] == "user":
        ...
        if "image" in msg:
            ...
            # Kesintisiz ve güvenli render için yerel st.image motoruna aktarıldı
...
if len(st.session_state.messages) > 0 and st.session_state.messages[-1]["role"] == "user":
    ...
    # GÖRSEL ÜRETİM SENARYOSU
    if any(trigger in user_query.lower() for trigger in image_triggers):
        with st.spinner(""):
            # İstediğin Sol Hizalı Mavi Balon İçi Yükleme Ekranı
            st.markdown(
            ...
            # Tetikleyicileri temizleyip saf komutu alma ve gerçekçilik parametrelerini ekleme
            ...
            # İstediğin gibi filtresiz/sansürsüz ve senin tarzında ultra gerçekçi ekleme yapıldı
            ...
            # Pollinations filtresiz motor bağlantısı
    ...
    # GEMINI METİN CEVAPLAMA SENARYOSU
    else:
